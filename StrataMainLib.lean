...
def printGlobalHelp (groups : List CommandGroup := commandGroups) : IO Unit := do
...
/-- Dispatch CLI arguments against a command map. This is the shared entry point
    that both the default executable and downstream custom executables use. -/
